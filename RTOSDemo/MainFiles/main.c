...
// Define whether to use the infrared sensor
#define USE_MTJ_V4INFRARED_Sensor 1
...
static vtSensorStruct infraredSensorData;
...
int main( void )
{
	...
	#if USE_MTJ_V4Infrared_Sensor == 1
	...
	#if USE_MTJ_LCD == 1
	vStarti2cSensorTask(&infraredSensorData,mainI2CSENSOR_TASK_PRIORITY,&vtI2C0,&vtLCDdata);
	#else
	vStarti2cTempTask(&infraredSensorData,mainI2CSENSOR_TASK_PRIORITY,&vtI2C0,NULL);
	#endif
	// Here we set up a timer that will send messages to the Sensor task.  The timer will determine how often the sensor is sampled
	startTimerForSensor(&infraredSensorData);
